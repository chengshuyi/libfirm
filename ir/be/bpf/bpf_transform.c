...
{
	return transform_binop(node, new_bd_bpf_And);
}

static ir_node *gen_Or(ir_node *node)
{
	return transform_binop(node, new_bd_bpf_Or);
}

static ir_node *gen_Eor(ir_node *node)
{
	return transform_binop(node, new_bd_bpf_Xor);
}
...
#endif
	return transform_binop(node, new_bd_bpf_fDiv);
}
...
static ir_node *gen_Shl(ir_node *node)
{
	...
	return transform_binop(node, new_bd_bpf_Shl);
}
...
static ir_node *gen_Shr(ir_node *node)
{
	...
	if (get_mode_modulo_shift(mode) != 32)
		panic("modulo shift!=32 not supported");
	return transform_binop(node, new_bd_bpf_Shr);
}
...
static ir_node *gen_Add(ir_node *node)
{
	...
	if (mode_is_float(mode)) {
		return transform_binop(node, new_bd_bpf_fAdd);
	}
	...
	return transform_binop(node, new_bd_bpf_Add);
}
...
static ir_node *gen_Sub(ir_node *node)
{
	...
	if (mode_is_float(mode)) {
		return transform_binop(node, new_bd_bpf_fSub);
	}
	return transform_binop(node, new_bd_bpf_Sub);
}
...
static ir_node *gen_Mul(ir_node *node)
{
	...
	if (mode_is_float(mode)) {
		return transform_binop(node, new_bd_bpf_fMul);
	}
	return transform_binop(node, new_bd_bpf_Mul);
}
...
static ir_node *gen_Minus(ir_node *node)
{
	...
	if (mode_is_float(mode)) {
		return transform_unop(node, n_Minus_op, new_bd_bpf_fMinus);
	}
	return transform_unop(node, n_Minus_op, new_bd_bpf_Minus);
}

static ir_node *gen_Not(ir_node *node)
{
	return transform_unop(node, n_Not_op, new_bd_bpf_Not);
}
...
static ir_node *gen_Store(ir_node *node)
{
	...
	if (mode_is_float(mode)) {
		return new_bd_bpf_fStore(dbgi, new_block, new_mem, new_ptr, new_val);
	}
	return new_bd_bpf_Store(dbgi, new_block, new_mem, new_ptr, new_val);
}
...
{
	be_start_out outs[N_BPF_REGISTERS] = { [REG_SP] = BE_START_IGNORE };
...
static ir_node *gen_Return(ir_node *node)
{
	...
	reqs[n_bpf_Return_stack] = &bpf_single_reg_req_gp_sp;
